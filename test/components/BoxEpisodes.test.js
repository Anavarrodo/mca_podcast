...
describe("test by <BoxEpisodes /> ", () => {
  ...
  test("handles episode clicks correctly", () => {
    render(
      <AppContext.Provider value={{ setCurrentLocation }}>
        <BoxEpisodes info={boxEpisodesMock} id={id} />
      </AppContext.Provider>
    );

    fireEvent.click(
      screen.getByText("'The Show' | Every Single Album: Niall Horan")
    );

    expect(mockNavigate).toHaveBeenCalledWith(
      "/podcast/1535809341/episode/011541a0-11e8-11ee-afbc-1b6aa7ef28c2",
      {
        state: {
