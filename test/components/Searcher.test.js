...
describe("test by <Searcher /> ", () => {
  test("renders without error", () => {
    const { getByTestId } = render(
      <Searcher defaultPodcasts={Array(100)} onChange={() => {}} />
    );
  ...
  test("display of the number of results", () => {
    const { getByTestId } = render(
      <Searcher defaultPodcasts={Array(58)} onChange={() => {}} />
    );
  ...
  test("search", () => {
    ...
    const { getByPlaceholderText } = render(
      <Searcher defaultPodcasts={searcherMock} onChange={onChangeMock} />
    );

    fireEvent.change(getByPlaceholderText("Filter podcasts"), {
      target: { value: "Rolling Sto" },
    });
